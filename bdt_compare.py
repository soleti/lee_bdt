...
for j in range(h_bdt_mc.GetNhists()):
    if h_bdt_mc.GetHists()[j].Integral():
        legend.AddEntry(h_bdt_mc.GetHists()[j], "%s: %.0f events" % (description[j], h_bdt_mc.GetHists()[j].Integral()), "f")

h_bdt_mc.Add(h_bdt_dataext)
...
legend.AddEntry(h_bdt_data, "Data BNB: %.0f events" % (h_bdt_data.Integral()), "lep")

legend.AddEntry(h_bdt_dataext, "Data EXT: %.0f events" % (h_bdt_dataext.Integral()), "f")
