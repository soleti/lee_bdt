...
print(mode, variation, BDT, MANUAL)
...
def fill_histos_data(tree_name, mode="nue"):
    f_data = ROOT.TFile("root_files_sys/%s_%s_file.root" % (tree_name, variation))
    ...
    f_clone = ROOT.TFile("root_files_sys/filtered_%s.root" % tree_name, "RECREATE")
    ...
    for h in histograms:
        f = ROOT.TFile("%splots%s/%s_%s.root" % (detsyst, folder, h.GetName(), tree_name),
                       "RECREATE")
        h.Write()
        f.Close()
...
def fill_histos(chain, histo_dict, h_bdt_types, option="", mode="nue"):
    ...
    for i in tqdm(range(entries)):
        ...
        if pre_cuts(variables_dict, numu):
            for bdt_name in bdt_types:
                h_bdt_types[bdt_name][category].Fill(
                    bdts[bdt_name], chain.event_weight)
        else:
            continue
...
if __name__ == "__main__":
    ...
    print("root_files_sys/mc_%s_file.root" % variation)
    mc_chain.Add("root_files_sys/mc_%s_file.root" % variation)
    # mc_chain.Add("root_files_sys/dirt%s_.root" % variation)

    # nue_chain.Add("root_files_sys/nue_%s_file.root" % variation)
    # bnbext_chain.Add("root_files_sys/bnbext_%s_file.root" % variation)
    ...
    for h in stacked_histos:
        f = ROOT.TFile("%splots%s/%s_mc.root" % (detsyst, folder, h.GetName()), "RECREATE")
        h.Write()
        f.Close()

    pickle.dump(h_pdgs, open("%splots%s/pdg_plots.p" % (detsyst, folder), "wb"))
